...
function displaySearchResults()
{
    ...
    if (isset($_GET['searchForm']))
    {
        ...
        if (!empty($_GET['priceStart']))
        {
            ...
            //$namedParamters[":priceStart"] = "%" . $_GET['priceStart'] . "%";
        } 
        if (!empty($_GET['priceEnd']))
        {
            // checks if user has typed something in product text box
            $sql .= " AND price <= " . $_GET['priceEnd'];
            //s$namedParamters[":priceEnd"] = "%" . $_GET['priceEnd'] . "%";
        }        
...
function displayAllProducts(){
    ...
    $sql = "SELECT * FROM sc_product ORDER BY team";
    $stmt = $dbConn->prepare($sql);
    $stmt->execute();
    ...
    foreach ($records as $record) {
        ...
        echo "   <input type='hidden' name='productId' value='".$record['prodId']."'>";
        echo "   <button type='submit'>Delete</button>";
        echo "</form>";
        echo "[<a 
        
        onclick='openModal()' target='productModal'
        href='productinfo.php?productId=".$record['prodId']."'>".$record['team']."</a>]  ";
...
function getProductInfo($productId) {
    ...
    $sql = "SELECT * FROM sc_product WHERE prodId = $productId";
    $stmt = $dbConn->prepare($sql);
    $stmt->execute();
